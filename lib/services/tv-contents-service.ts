...
import { TvContent, CreateTvContentData, UpdateTvContentData } from '../../types/tv';
...
export class TvContentsService {
  ...
  static async getContents(categoryId: string): Promise<TvContent[]> {
    try {
      const contentsRef = collection(db, TV_CATEGORIES_COLLECTION, categoryId, TV_CONTENTS_SUBCOLLECTION);
  ...
  static async getContent(categoryId: string, contentId: string): Promise<TvContent | null> {
    try {
      const contentRef = doc(db, TV_CATEGORIES_COLLECTION, categoryId, TV_CONTENTS_SUBCOLLECTION, contentId);
  ...
  static async createContent(categoryId: string, data: CreateTvContentData): Promise<TvContent> {
    try {
      return await runTransaction(db, async (transaction) => {
        const docRef = doc(collection(db, TV_CATEGORIES_COLLECTION, categoryId, TV_CONTENTS_SUBCOLLECTION));
  ...
  static async updateContent(categoryId: string, contentId: string, data: UpdateTvContentData): Promise<void> {
    try {
  ...
  // Delete a content
  static async deleteContent(categoryId: string, contentId: string): Promise<void> {
    try {
      await runTransaction(db, async (transaction) => {
        const contentRef = doc(db, TV_CATEGORIES_COLLECTION, categoryId, TV_CONTENTS_SUBCOLLECTION, contentId);
        const docSnap = await transaction.get(contentRef);
