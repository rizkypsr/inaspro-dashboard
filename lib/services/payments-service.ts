...
class PaymentsService {
  ...
  async getPaymentsByStatus(status: "PENDING" | "PAID" | "FAILED" | "EXPIRED"): Promise<Payment[]> {
    try {
      const q = query(
        collection(db, this.collectionName),
        where('status', '==', status),
  ...
  async getPaymentByRegistrationId(registrationId: string): Promise<Payment | null> {
    try {
      const q = query(
        collection(db, this.collectionName),
  ...
  async getPaymentsByMethod(paymentMethod: string, fantasyId?: string): Promise<Payment[]> {
    try {
      let q;
      if (fantasyId) {
        q = query(
