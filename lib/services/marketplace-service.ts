...
export class ProductsService {
  ...
  async getProducts(filters?: ProductFilters, pagination?: PaginationParams): Promise<PaginatedResponse<Product>> {
    ...
    return {
      ...
      totalPages: Math.ceil(filteredProducts.length / (pagination?.limit || 10))
    };
  }
...
export class CategoriesService {
  ...
  async updateCategory(categoryId: string, data: Partial<CreateCategoryData>): Promise<void> {
    const docRef = doc(db, this.collectionName, categoryId);
...
export class OrdersService {
  ...
  async getOrders(filters?: OrderFilters, pagination?: PaginationParams): Promise<PaginatedResponse<Order>> {
    let q = query(collection(db, this.collectionName));
...
export class VouchersService {
  ...
  async createVoucher(data: CreateVoucherData): Promise<string> {
    ...
    const docRef = await addDoc(collection(db, this.collectionName), voucherData);
    return docRef.id;
  }

  async updateVoucher(voucherId: string, data: Partial<CreateVoucherData>): Promise<void> {
...
export class LogisticsService {
  ...
  async updateLogisticsRate(provinceId: string, data: Omit<LogisticsRate, 'provinceId'>): Promise<void> {
...
export class NotificationsService {
  ...
  async createNotification(data: Omit<Notification, 'notifId' | 'createdAt'>): Promise<string> {
