...
import { TvCategory, CreateTvCategoryData, UpdateTvCategoryData } from '../../types/tv';
...
export class TvCategoriesService {
  ...
  static async updateCategory(id: string, data: UpdateTvCategoryData): Promise<void> {
    try {
  ...
  static async deleteCategory(id: string): Promise<void> {
    try {
      await runTransaction(db, async (transaction) => {
        ...
        const contentsQuery = query(collection(db, TV_CATEGORIES_COLLECTION, id, 'contents'));
        const contentsSnapshot = await getDocs(contentsQuery);
        
        contentsSnapshot.docs.forEach((contentDoc) => {
          transaction.delete(doc(db, TV_CATEGORIES_COLLECTION, id, 'contents', contentDoc.id));
        });
