...
const ShoppingCartIcon = () => (
  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 3h2l.4 2M7 13h10l4-8H5.4m0 0L7 13m0 0l-2.293 2.293A1 1 0 004 16v2a1 1 0 001 1h1m5-6v6a1 1 0 001 1h2a1 1 0 001-1v-6m3 0V9a1 1 0 00-1-1h-2a1 1 0 00-1 1v4.01" />
  </svg>
);
...
export default function NotificationsPage() {
  ...
  const [filter, setFilter] = useState<'all' | 'unread' | 'order' | 'payment' | 'stock'>('all');
  ...
  const calculateStats = () => {
    ...
    const paymentNotifications = notifications.filter(n => n.type === 'payment').length;
    const stockNotifications = notifications.filter(n => n.type === 'stock').length;
      ...
      <div className="p-6 max-w-7xl mx-auto">
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <p className="text-default-500">Stay updated with orders, payments, and stock alerts</p>
          </div>
          ...
          <Card className={filter === 'payment' ? 'ring-2 ring-primary' : ''}>
            ...
            >
              ...
              <div>
                ...
                <p className="text-2xl font-bold">{stats.paymentNotifications}</p>
              </div>
            </CardBody>
          </Card>
            ...
            {loading ? (
              ...
              <div className="text-center py-8 px-6">
                ...
                <p className="text-sm text-default-400">
                  Notifications will appear here when there are new orders, payments, or stock alerts
                </p>
              </div>
            ) : (
                  ...
                  >
                    <div className="flex items-start gap-4">
                      <div className={`p-2 rounded-lg ${
                        getNotificationColor(notification.type) === 'primary' ? 'bg-primary/10' :
                        getNotificationColor(notification.type) === 'success' ? 'bg-success/10' :
                        getNotificationColor(notification.type) === 'warning' ? 'bg-warning/10' :
                        'bg-default/10'
                      }`}>
                        ...
                        <div className="flex items-start justify-between gap-2">
                          <div className="flex-1">
                            ...
                            <p className={`text-sm mb-2 ${
                              !notification.isRead ? 'text-default-700' : 'text-default-500'
                            }`}>
                          ...
                          <div className="flex items-center gap-2">
                            {!notification.isRead && (
                              <Button
                                ...
                                color="primary"
                                startContent={<CheckIcon />}
                                onPress={() => handleMarkAsRead(notification.notifId)}
                              >
                                Mark Read
                              </Button>
                            )}
