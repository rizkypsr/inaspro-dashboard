...
export default function LogisticsPage() {
  ...
  const [selectedLogistics, setSelectedLogistics] = useState<LogisticsRate | null>(null);
  ...
  const handleDelete = async (logisticsConfig: LogisticsRate) => {
    if (!confirm(`Are you sure you want to delete logistics configuration for "${logisticsConfig.name}"?`)) {
      return;
    }
  ...
  const handleSubmit = async () => {
    ...
    if (!isEditing) {
      const existingProvince = logistics.find(l => 
    ...
    try {
      ...
      if (isEditing && selectedLogistics) {
        await logisticsService.updateLogisticsRate(selectedLogistics.provinceId, logisticsData);
  ...
  const getAvailableProvinces = () => {
    ...
    return INDONESIAN_PROVINCES.filter(province => 
      ...
      <div className="p-6 max-w-7xl mx-auto">
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <p className="text-default-500">Manage shipping costs by province</p>
          </div>
            ...
            <CardBody className="flex flex-row items-center gap-4">
              ...
              <div>
                ...
                <p className="text-2xl font-bold">
                  {logistics.length > 0 
                    ? formatCurrency(logistics.reduce((sum, l) => sum + l.price, 0) / logistics.length)
                    : 'Rp 0'
                  }
                </p>
              </div>
          ...
          <Card>
            <CardBody className="flex flex-row items-center gap-4">
              ...
              <div>
                ...
                <p className="text-2xl font-bold">{INDONESIAN_PROVINCES.length - logistics.length}</p>
              </div>
            </CardBody>
          </Card>
        </div>
        ...
        <Card>
          <CardHeader>
            <h3 className="text-lg font-semibold">Logistics Configurations ({filteredLogistics.length})</h3>
          </CardHeader>
          <CardBody>
            {loading ? (
              ...
              <div className="text-center py-8">
                <p className="text-default-500 mb-4">
                  {searchTerm ? 'No provinces found matching your search' : 'No logistics configurations found'}
                </p>
                {!searchTerm && (
                  <Button color="primary" startContent={<PlusIcon />} onPress={handleAdd}>
                    Add First Province
