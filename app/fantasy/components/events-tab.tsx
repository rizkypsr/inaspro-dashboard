...
import { fantasyService, type Fantasy, type CreateFantasyData } from '@/lib/services/fantasy-service';
...
export default function EventsTab() {
  ...
  const handleView = (event: Fantasy) => {
    setSelectedEvent(event);
    setIsEditing(false);
    setFormData({
      title: event.title,
  ...
  const handleViewDetails = (eventId: string) => {
    router.push(`/fantasy/events/${eventId}`);
  };
    ...
    try {
      if (selectedEvent && selectedEvent.id) {
        ...
        const updateData: Partial<CreateFantasyData> = {
          ...
          schedule: formData.schedule || timestampToDate(selectedEvent.schedule)
        };
  ...
  return (
    <div className="space-y-6">
                  ...
                  <TableRow key={event.id}>
                    ...
                    <TableCell>
                      <div className="text-sm">
                        <p>{timestampToDate(event.schedule).toLocaleDateString()}</p>
                        <p className="text-gray-500">{timestampToDate(event.schedule).toLocaleTimeString()}</p>
                      </div>
                    </TableCell>
                    <TableCell>{event.venue}</TableCell>
                    <TableCell>{event.address}</TableCell>
                    <TableCell>Rp {event.registrationFee.toLocaleString()}</TableCell>
                    <TableCell>
                      <Chip
                        size="sm"
                  ...
                  <TableCell>
                    <div className="flex gap-2">
                      <Button
                        ...
                        size="sm"
                        variant="light"
                        onPress={() => event.id && router.push(`/fantasy/events/${event.id}`)}
                      >
                        <EyeIcon />
                      </Button>
                      <Button
      ...
      <Modal isOpen={isOpen} onClose={onClose} size="2xl">
        <ModalContent>
          <ModalHeader>
            {selectedEvent ? (isEditing ? 'Edit Event' : 'View Event') : 'Add New Event'}
          </ModalHeader>
          <ModalBody>
            <div className="space-y-4">
              ...
              <Input
                label="Schedule"
                type="datetime-local"
                value={formData.schedule ? formData.schedule.toISOString().slice(0, 16) : ''}
                onChange={(e: any) =>
              ...
              <div className="grid grid-cols-2 gap-4">
                <Input
                  ...
                  onChange={(e: any) =>
                    setFormData({ ...formData, registrationFee: parseInt(e.target.value) || 0 })
                  }
                  isReadOnly={!!(selectedEvent && !isEditing)}
                />
                <div className="flex items-center gap-2">
                  <label className="text-sm font-medium">International Event</label>
                  <input
                    type="checkbox"
